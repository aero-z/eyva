...
/* This method copies everything from `src' to `dest' until a newline appears:
 */
...
void Server::handleIncomingData(int id) {
	...
	if(received == 0) {
		logf(NORMAL_LOG, "> %s: connection closed by peer",
				clients[id]->getIP());
		/* TODO
		connection_handler->disconnect(clients[id]->getSocket());
		*/
	 ...
	 */
		 ...
		 */
			 ...
			 */
			connection_handler->process(output_buffer, input_buffer,
					BUFFER_SIZE);
...
void Server::handleStdInput(void) {
	...
	if(strstr(command, "help") == command) {
		...
		printf("  \e[32mquit\e[0m   shut the server down\n");
	}
}
